export function getPredytube(): any[] {
  const predytube: any[] = [
    {
      id: "25143269-58ce-4fb7-ae94-5bad3333ab02",
      Titulo: "Eso es todo amigos",
      thumbnail: "",
      fechaSalida: "2016-04-20",
      VisualizacionesYoutube: 510,
      descripcion:
        "Bueno, ya se ha acabado el proyecto, era lógico ¿no? también por otra parte... Pero no os preocupéis porque voy a seguir subiendo vídeos al canal, pero sin respetar el hecho de sacarlos cada miércoles. Los nuevos vídeos vendrán en función de mi disponibilidad, no olvidéis suscribiros para estar al tanto de mis locuras. Espero que os lo hayáis pasado genial con este proyecto llamado Dyferente y en breves vuelvo con locuras nuevas! :P Un abrazo enorme chavalada!!! :P",
      descripcionSugerida:
        "'Eso es todo amigos' marca el final de una era y el comienzo de otra. Aunque Dyferente se despide, las aventuras continúan sin calendario fijo. ¡Prepárate para más locuras y asegúrate de seguir conectado!",
      featuring: "",
      beat: "",
      mezcla: "",
      originalLikes: 32,
      material: "Eso es todo amigos",
    },
    {
      id: "25143269-58ce-4fb7-ae94-5bad3333ab02",
      Titulo: "El placer de los artistas",
      thumbnail: "",
      fechaSalida: "2016-04-13",
      VisualizacionesYoutube: 802,
      descripcion:
        "Y volvemos con mas!, aprovechamos la barbacoa, los amigos, el buen rollo y el cumpleaños de Chucky para grabar un Predytube de como fue nuestra tarde. Este vídeo no hubiera sido posible sin Tacho. Espero que lo disfrutéis y solo me queda mandar un beso a todos  (ya que hoy es el día del Beso) y en especial a los que han participado en el clip, costa, alex, iff, chucky etc...",
      descripcionSugerida:
        "En 'El placer de los artistas', capturamos la esencia de un día perfecto: amigos, música y barbacoa. Una celebración de la vida y la creatividad, este Predytube es un tributo a los momentos que nos unen.",
      featuring: "",
      beat: "",
      mezcla: "Cansino (Nunca Piso Freno)",
      originalLikes: 33,
      material: "El placer de los artistas",
    },
    {
      id: "25143269-58ce-4fb7-ae94-5bad3333ab02",
      Titulo: "Predytube especial versión South Park",
      thumbnail: "",
      fechaSalida: "2016-04-06",
      VisualizacionesYoutube: 719,
      descripcion:
        "Hoy toca un vídeo especial, celebrando que ya hemos pasado los 10 Predytubes, he decidido hacer un videoclip en formato South Park.",
      descripcionSugerida:
        "Celebrando nuestro décimo Predytube con un giro único: una edición especial al estilo South Park. Este hito creativo fusiona el humor irreverente de la serie con nuestro sello distintivo, en un homenaje animado que no querrás perderte.",
      featuring: "Dj Xino",
      beat: "",
      mezcla: "Cansino (PlaymoBeats)",
      originalLikes: 52,
      material: "Especial version South Park",
    },

    {
      id: "25143269-58ce-4fb7-ae94-5bad3333ab02",
      Titulo: "No vas a creer lo que hace Predy en el bosque",
      thumbnail: "",
      fechaSalida: "2016-03-30",
      VisualizacionesYoutube: 599,
      descripcion:
        "Un segundo es una foto, de esa idea nace el Predytube de esta semana.",
      descripcionSugerida:
        "Explorando la esencia de los momentos, 'No vas a creer lo que hace Predy en el bosque' captura la magia efímera de la naturaleza y nuestra conexión con ella. Una aventura donde cada segundo cuenta una historia única.",
      featuring: "Lorena Giner",
      beat: "",
      mezcla: "Cansino (PlaymoBeats)",
      originalLikes: 43,
      material: "No vas a creer lo que hace Predy en el bosque",
    },
    {
      id: "25143269-58ce-4fb7-ae94-5bad3333ab02",
      Titulo: "Sacado de la patilla",
      thumbnail: "",
      fechaSalida: "2016-03-23",
      VisualizacionesYoutube: 379,
      descripcion:
        "¿Queríais espontaneidad? ¿algo fresco? Pues que mejor que un freestyle, recién sacado de la patilla!!! xD, No es una improvisación perfecta, pero bueno, almenos tiene patas y se va arrastrando por el suelo mientras emite algún que otro sonido.",
      descripcionSugerida:
        "'Sacado de la patilla' ofrece un vistazo a la improvisación en su forma más cruda y auténtica. Un freestyle que, aunque imperfecto, rebosa de verdad y espíritu, demostrando que la perfección reside en la expresión genuina.",
      featuring: "",
      beat: "IduBeats",
      mezcla: "",
      originalLikes: 30,
      material: "Sacado de la patilla",
    },
    {
      id: "25143269-58ce-4fb7-ae94-5bad3333ab02",
      Titulo: "Sopa de Letras",
      thumbnail: "",
      fechaSalida: "2016-03-17",
      VisualizacionesYoutube: 238,
      descripcion:
        "El Predytube de esta semana es un lyric vídeo bajo el contexto de un tema buen rollero para sacaros una sonrisa.",
      descripcionSugerida:
        "'Sopa de Letras' es nuestro regalo lírico para ti, un vídeo lleno de letras que bailan al ritmo de un buen rollo contagioso. Diseñado para iluminar tu día con una sonrisa, este lyric video te invita a sumergirte en un mar de palabras.",
      featuring: "",
      beat: "",
      mezcla: "",
      originalLikes: 11,
      material: "Sopa de Letras",
    },
    {
      id: "25143269-58ce-4fb7-ae94-5bad3333ab02",
      Titulo: "¿Aún existe la palabra toyako?",
      thumbnail: "",
      fechaSalida: "2016-03-09",
      VisualizacionesYoutube: 946,
      descripcion:
        "¿Aún existe la palabra Toyako? Toyako es una palabra que se utilizaba en los inicios de el Hip-hop para desprestigiar a alguien, (es un adjetivo de Incompetente) el vídeo esta enfocado en un graffiti puesto que es en el ámbito donde se suele utilizar mas este calificativo despectivo.",
      descripcionSugerida:
        "Reflexionando sobre el lenguaje del hip-hop, '¿Aún existe la palabra toyako?' es un viaje cultural que explora el impacto de las palabras en el arte del graffiti. Un diálogo entre pasado y presente, buscando el significado detrás de las etiquetas.",
      featuring: "",
      beat: "",
      mezcla: "",
      originalLikes: 34,
      material: "Aun existe la palabra toyako",
    },

    {
      id: "25143269-58ce-4fb7-ae94-5bad3333ab02",
      Titulo: "Estruc tuc tuc tuc turas",
      thumbnail: "",
      fechaSalida: "2016-03-02",
      VisualizacionesYoutube: 560,
      descripcion:
        "Esta vez desde Predytube nos hemos enfocado en una parte muy concreta de la música, se trata de la métrica. Así que aquí tenéis un montón de estruc tuc tuc tuc turas! que las disfrutéis!",
      descripcionSugerida:
        "'Estruc tuc tuc tuc turas' despliega el arte de la métrica en su máxima expresión, llevándote a través de un laberinto rítmico que desafía lo convencional. Sumérgete en esta exploración sonora donde cada 'tuc' es un paso hacia lo inesperado.",
      featuring: "",
      beat: "Fx M Black Beats",
      mezcla: "",
      originalLikes: 28,
      material: "Estruc tuc tuc tuc turas",
    },
    {
      id: "25143269-58ce-4fb7-ae94-5bad3333ab02",
      Titulo: "Dos colgaos + Francia + Flow = Flower",
      thumbnail: "",
      fechaSalida: "2016-02-24",
      VisualizacionesYoutube: 350,
      descripcion:
        "Esta vez os traemos un Predytube algo internacional, desde Francia, concretamente la ciudad de Lyon llega con un estilo mas fresco el videoclip de Flower ambientado en una estética comic para desarrollar este viaje trambolikoh!",
      descripcionSugerida:
        "En 'Dos colgaos + Francia + Flow = Flower', nos embarcamos en una aventura internacional hasta Lyon, fusionando flow con el encanto francés en un videoclip que rebosa estilo y frescura. Prepárate para un viaje visual y musical inolvidable, adornado con una estética cómic.",
      featuring: "",
      beat: "",
      mezcla: "",
      originalLikes: 20,
      material: "Flower",
    },
    {
      id: "25143269-58ce-4fb7-ae94-5bad3333ab02",
      Titulo: "20 SONGS TAG CHALLENGE",
      thumbnail: "",
      fechaSalida: "2016-02-17",
      VisualizacionesYoutube: 438,
      descripcion:
        "Pues nada, siguiente, el siguiente vídeo se trata de un challenge, el de 20 songs tag, o el tag de las 20 canciones, se trata de revelar un cuestionario de preguntas relacionadas con la música.",
      descripcionSugerida:
        "Aceptando el '20 SONGS TAG CHALLENGE', me sumerjo en el desafío de revelar mi alma musical a través de un cuestionario que atraviesa mi banda sonora personal. Es una invitación a conocer los ritmos que marcan mi camino, un viaje íntimo a través de veinte canciones inolvidables.",
      featuring: "",
      beat: "",
      mezcla: "",
      originalLikes: 35,
      material: "20Songs",
    },
    {
      id: "25143269-58ce-4fb7-ae94-5bad3333ab02",
      Titulo: "Aire",
      thumbnail: "",
      fechaSalida: "2016-02-10",
      VisualizacionesYoutube: 522,
      descripcion:
        "En esta ocasión #Predytube os trae algo #Dyferente como siempre, esta vez es la versión melódica del tema de Aire que vio la luz en el año 2013.",
      descripcionSugerida:
        "'Aire', en su nueva versión melódica, te envuelve con su brisa suave, llevándote de vuelta al 2013 con una frescura renovada. Es un respiro, una pausa musical que invita a la reflexión y al disfrute puro.",
      featuring: "",
      beat: "Jay Beat",
      mezcla: "Cansino (Playmobeats)",
      originalLikes: 44,
      material: "Aire 2016",
    },

    {
      id: "25143269-58ce-4fb7-ae94-5bad3333ab02",
      Titulo: "GamePlay de Rap en Español",
      thumbnail: "",
      fechaSalida: "2016-02-03",
      VisualizacionesYoutube: 546,
      descripcion:
        "Ahora usando un formato nunca antes visto en una canción, se trata de un game play de Rap, en la que tanto el videojuego como el jugador van acorde con la canción del clip.",
      descripcionSugerida:
        "'GamePlay de Rap en Español' fusiona el mundo de los videojuegos con el ritmo del rap, creando una sinergia única donde la música y la acción se complementan a la perfección. Es una experiencia inmersiva que redefine lo que un videoclip puede ser.",
      featuring: "David",
      beat: "",
      mezcla: "",
      originalLikes: 33,
      material: "GamePlay de Rap",
    },
    {
      id: "25143269-58ce-4fb7-ae94-5bad3333ab02",
      Titulo: "El mejor Thug Life RAP en Español!",
      thumbnail: "",
      fechaSalida: "2016-01-27",
      VisualizacionesYoutube: 1501,
      descripcion:
        "Baung!! Predy trae el mejor Gangsta Rap al micro chavales, ¿Qué porqué es el mejor? Porque me lo ha dicho mi abuela.",
      descripcionSugerida:
        "'El mejor Thug Life RAP en Español!' nos lleva al corazón del gangsta rap con una confianza que solo puede venir del respaldo de una abuela. Es una oda a las raíces más duras del género, entregada con un guiño y la autenticidad de quien sabe que tiene algo especial.",
      featuring: "",
      beat: "California Livin",
      mezcla: "",
      originalLikes: 57,
      material: "El mejor Thug Life RAP",
    },
    {
      id: "25143269-58ce-4fb7-ae94-5bad3333ab02",
      Titulo: "Llega Predytube!!!",
      thumbnail: "",
      fechaSalida: "2016-01-20",
      VisualizacionesYoutube: 1922,
      descripcion:
        "Vuelve Predy Y eso no es todo amigos!, porque comienza Predytube, cada miércoles tendréis vídeo en este canal así que Suscribiros y uniros a la familia ;)!",
      descripcionSugerida:
        "'Llega Predytube!!!' marca el emocionante lanzamiento de una serie que promete acercarte a mi mundo como nunca antes. Con la promesa de nuevos vídeos cada miércoles, este es el inicio de un viaje que nos mantendrá conectados, riendo y explorando juntos. Únete a la familia y sé parte de cada sorpresa que tengo preparada.",
      featuring: "",
      beat: "",
      mezcla: "Cansino (PlaymoBeats)",
      originalLikes: 69,
      material: "Llega Predytube",
    },
  ]

  return predytube
}
